...
module.exports = {
  sendCallback (msg, item) {
    ...
    switch (item) {
      case 'bouquets':
      case 'compose':
        text = `Хотите заказать ${item === 'compose' ? 'композицию' : 'букет'}?\nВы можете отсортировать ${item === 'compose' ? 'композиции' : 'букеты'} по стоимости или посмотреть весь ассортимент`;
        keyboard = {
          inline_keyboard: [
            [{text: `🎉 Выбрать повод`, callback_data: item === 'compose' ? 'c_reasons' : 'b_reasons'}],
            [{text: `💰 Выбрать по цене`, callback_data: item === 'compose' ? 'c_price' : 'b_price'}],
      ...
      case 'gifts':
        ...
        keyboard = {
          inline_keyboard: [
            [{text: `💰 Выбрать по цене`, callback_data: 'g_price'}],
